...
object_t eval(object_t obj,struct env *env)
{
	...
	case OBJ_PRIMITIVE:
		return eval_primitive(op,cdr_op(obj),env);
	case OBJ_PROCEDURE:
		return eval_procedure(op,cdr_op(obj),env);
	case OBJ_MACRO:
		return eval_macro();
	default:
		return sys_error(1);
...
object_t eval_syntax(struct syntax *op,object_t form,struct env *env)
{
	...
	case SYNTAX_BEGIN:
		return eval_begin(form,env);
	case SYNTAX_LAMBDA:
		return eval_lambda(form,env);
	case SYNTAX_SET:
		return eval_set(form,env);
	case SYNTAX_DEFINE:
...
object_t eval_begin(object_t form,struct env *env)
{
	if(form->type == OBJ_PAIR)
	{
		if(((struct pair*)form)->cdr == scheme_null)
			return eval(((struct pair*)form)->car,env);
		else
		{
			eval(((struct pair*)form)->car,env);
			return eval_begin(((struct pair*)form)->cdr,env);
		}
	}
	else if(form == scheme_null)
		return scheme_unspecified;
	else
		return sys_error(7);
}
object_t eval_set(object_t form,struct env *env)
{
	struct symbol *name;
	object_t value;
	object_t env_cell;
	...
	if(env_cell == scheme_false)
		return sys_error(2);
	value = eval(cadr_op(form),env);
	if(value->type == OBJ_ERROR)
		return value;
	return set_cdr_op(env_cell,value);
}
object_t eval_lambda(object_t form,struct env *env)
{
	object_t variables;
	object_t body;
	...
	return make_procedure(variables,body,env);
}
...
object_t eval_argument(object_t form,struct env *env)
{
	if(form == scheme_null)
		return scheme_null;
	else if(form->type == OBJ_PAIR)
	{
		return cons_op(eval(((struct pair*)form)->car,env),eval_argument(((struct pair*)form)->cdr,env));
	}
	else
		return sys_error(9);
}
object_t eval_primitive(struct primitive *f,object_t form,struct env *env)
{
	object_t values = eval_argument(form,env);
	if(length(values) != f->arg_num)
		return sys_error(12);
	switch(f->arg_num)
	{
	case 0:
		return (f->callback.op0)();
	case 1:
		return (f->callback.op1)(car_op(values));
	case 2:
		return (f->callback.op2)(car_op(values),cadr_op(values));
	case 3:
		return (f->callback.op3)(car_op(values),cadr_op(values),caddr_op(values));
	case 4:
		return (f->callback.op4)(car_op(values),cadr_op(values),caddr_op(values),cadddr_op(values));
	case 5:
		return (f->callback.op5)(car_op(values),cadr_op(values),caddr_op(values),cadddr_op(values),list_ref(values,4));
	case 6:
		return (f->callback.op6)(car_op(values),cadr_op(values),caddr_op(values),cadddr_op(values),list_ref(values,4),list_ref(values,5));
	}
}
...
object_t eval_procedure(struct procedure *f,object_t form,struct env *env)
{
	object_t values = eval_argument(form,env);
