  ...
  (cond 
   [(symbol? x)
	...
	(let ([a (environment-ref sandbox x)])
	  (if (not a)
	      (s-error #f "compile error: unbound variable" x)
	      (lambda v
		(if (eq? (cdr a) not-bound)
		    (s-error #f "runtime error: unbound variable" x)
		    (cdr a)))))]
		 ...
		 (let ([val (compile (caddr x) e sandbox)])
		   (cond 
		    [(not i)
		     (let ([a (environment-ref sandbox var)])
		       (if (not a)
			   (s-error #f "compile error: set! unbound variable" var)
			   (let ([val (compile (caddr x) e sandbox)])
		 ...
		 (cond
		  [(not i)
		   (let ([a (environment-ref sandbox var)])
		     (if (not a)
			 (begin
			   (environment-set! sandbox var not-bound)
			   (set! a (environment-ref sandbox var))))		     		     
		     (let ([val (compile (caddr x) e sandbox)])
		       (lambda (v) (set-cdr! a (val v)))))]
		  [(zero? i)
		   (let ([val (compile (caddr x) e sandbox)])
