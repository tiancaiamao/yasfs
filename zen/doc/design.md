...
lua既然可以所有东西都是table，那我也可以所有东西都是tuple!!!

模块也可以是tuple！！！

引入这种模块之后，一个问题是编译成多个bytecode文件，加载之后不能再用单纯的pc寻址，必须改成module+pc的形式，因为
单纯的pc不能确定是用的哪个模块了。

改起来也不麻烦，无非是原来的保留返回地址是保存的pc，再在保存addr=module+pc。可以用64字节来存，高位留给module，低位用作pc
留8192个module应该够了。

还引入的另一个问题是tuple的全局性。如果tag是全局的，那么在一个模块里面定义
(define some (tuple Some 3))
另一个模块里面多了一个域，怎么搞？
(define some (tuple Some 1 2 3))
如果tag是全局的，就必须做到提前能知道其它模块会使用什么tag，这做不到

所以tag是局部的。tag是局部之后，一个模块里面定义的tuple如何给其它模块用？
...
我没有得到lisp的精神...lisp的真正精神应该是：代码即数据。

真正的精神是自己实现自己。真正的精髓应该是宏，像owl，像shen...这些语言是得到的lisp精髓的

不介意多很多层的抽象，lisp做它自己的，就只有lambda演算...没有类型系统...而C那边可以关注性能，可以优化GC等等

C的精髓是性能，对底层的控制能力

lisp的语法一点都不友好，上层做类型系统，之后可以转化成lisp的输入，lisp本来就是无类型的

模式匹配是宏的根本，因为宏需要用模式匹配来实现

F#的ast转成sexp -> sexp编译成bytecode -> bytecode再通过zinc虚拟机执行
而它自己也是运行在zinc之上的

所有的东西都可以变成s表达式，所有的s表达式都等价于可扫行的
