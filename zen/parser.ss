      ...
      (tuple Fun1 (length (car tl))
             (let ((nenv (extend-env env (cadr exp))))
               (map
                ...
                (cdr tl)))))
     ((eq? hd 'let)
      (let* ((args (map car (car tl)))
             (nenv (extend-env env args))
             (header (map
                      (lambda (x)
                        (tuple Set (find-env nenv (car x))
                             (parse (cadr x) env)))
                      (car tl)))
             (body (map (lambda (x) (parse x nenv)) (cdr tl))))
        (tuple Let (length args) (append header body))))
     ((eq? hd 'ccall)
