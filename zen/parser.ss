    ...
    (cond
     ((eq? hd 'if)
      (tuple If (parse (car tl) env)
          ...
          (parse (caddr tl) env)))
     ((eq? hd '+)
      (tuple Plus (parse (car tl) env) (parse (cadr tl) env)))
     ((eq? hd '-)
      (tuple Sub (parse (car tl) env) (parse (cadr tl) env)))
     ((eq? hd '*)
      (tuple Mul (parse (car tl) env) (parse (cadr tl) env)))
     ((eq? hd '/)
      (tuple Div (parse (car tl) env) (parse (cadr tl) env)))
     ((eq? hd 'load)
      ...
      (tuple Fun1 (length (car tl))
            (let ((nenv (extend-env env (cadr exp))))
              (map
               ...
               (cdr tl)))))
     ((eq? hd '=)
      (tuple Equal (parse (car tl) env) (parse (cadr tl) env)))
     ;; ((eq? hd 'let)
